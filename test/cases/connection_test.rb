...
class FieldWithConnections < GQL::Field
  ...
  connection :default_list_field_class, -> { FieldWithConnections::ITEMS }, item_field_class: ConnectionItemClass
...
class ConnectionTest < GQL::TestCase
  ...
  test "edges field returns nils if accessed without subfields" do
    value = GQL.execute('{ default_list_field_class { edges } }')

    assert_equal [nil, nil], value[:default_list_field_class][:edges]
  end

  test "edges field returns values if accessed with subfields" do
    value = GQL.execute('{ default_list_field_class { edges { foo } } }')

    assert_equal [{ foo: 'bar' }, { foo: 'baz' }], value[:default_list_field_class][:edges]
  end

  test "respects custom list class" do
    value = GQL.execute('{ custom_list_field_class { count, edges { foo } } }')

    assert_equal 2, value[:custom_list_field_class][:count]
    assert_equal [{ foo: 'bar' }, { foo: 'baz' }], value[:custom_list_field_class][:edges]
  end
end
